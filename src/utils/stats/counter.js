...
export async function getRuleCount(filePath) {
    try {
        ...
        await logMessage(`Error counting rules in ${filePath}: ${error.message}`, LogLevel.ERROR);
        return 0;
    }
}
...
export async function generateStats(paths) {
    ...
    try {
        ...
        await logMessage(`Failed to generate statistics: ${error.message}`, LogLevel.ERROR);
        throw error;
    }
}
...
export class UpdateCounter {
    ...
    recordDuplicate(originalRule, duplicateRule, type, source) {
        ...
        logMessage(`Duplicate found: ${duplicateRule} matches ${originalRule} (${type} from ${source})`, LogLevel.DEBUG);
    }
    ...
    async isValidUrl(url) {
        try {
            if (!url || typeof url !== 'string') {
                await logMessage(`Invalid URL (empty or wrong type): ${url}`, LogLevel.DEBUG);
                return false;
            }
            ...
            const domainPattern = /^([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)+[a-z]{2,}(\/[\w\-._~:/?#[\]@!$&'()*+,;=]*)?$/i;
            ...
            await logMessage(`URL validation error for ${url}: ${error.message}`, LogLevel.DEBUG);
            return false;
        }
    }
    ...
    async isValidSourceUrl(url) {
        try {
            ...
            if (!trustedDomains.some(domain => hostname === domain || hostname.endsWith(`.${domain}`))) {
                await logMessage(`Untrusted source domain: ${hostname}`, LogLevel.DEBUG);
                return false;
            }
            ...
            await logMessage(`Source URL validation error: ${error.message}`, LogLevel.DEBUG);
            return false;
        }
    }
    ...
    updateRuleCounts(counts) {
        ...
        this.stats.rules.total = Object.values(counts).reduce((a, b) => a + (b || 0), 0);
    }
    ...
    async printStats() {
        ...
        await logMessage(`- AdGuard Rules: ${stats.rules.valid.adguard}`, LogLevel.INFO);
        await logMessage(`- Hosts Rules: ${stats.rules.valid.hosts}`, LogLevel.INFO);
        await logMessage(`- Browser Rules: ${stats.rules.valid.browser}`, LogLevel.INFO);
        await logMessage(`- DNS Rewrite Rules: ${stats.rules.valid.dnsrewrite}`, LogLevel.INFO);
        await logMessage(`Skipped Rules: ${stats.rules.skipped}`, LogLevel.INFO);
        await logMessage(`Duplicate Rules: ${stats.rules.duplicates}`, LogLevel.INFO);
        await logMessage(`Invalid Rules: ${stats.rules.invalid}`, LogLevel.INFO);
        
        if (stats.sources.failed > 0) {
            await logMessage(`\nFailed Sources: ${stats.sources.failed}`, LogLevel.WARN);
